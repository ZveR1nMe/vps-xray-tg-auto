...
class XrayManager:
    ...
    def get_clash_config(self, client_uuid: str, name: str) -> str:
        """Генерирует YAML-конфиг для Clash Verge Rev с split-tunneling."""
        return (
            f"# Clash Verge Rev — {name}\n"
            f"# РФ напрямую, остальное через VPN\n"
            f"\n"
            f"proxies:\n"
            f"  - name: \"{name}\"\n"
            f"    type: vless\n"
            f"    server: {self._server_ip}\n"
            f"    port: 443\n"
            f"    uuid: {client_uuid}\n"
            f"    network: tcp\n"
            f"    tls: true\n"
            f"    udp: true\n"
            f"    flow: xtls-rprx-vision\n"
            f"    servername: {self._sni}\n"
            f"    reality-opts:\n"
            f"      public-key: {self._public_key}\n"
            f"      short-id: {self._short_id}\n"
            ...
            f"  fallback:\n"
            f"    - {self._remote_doh}\n"
            f"    - https://1.0.0.1/dns-query\n"
            f"  fallback-filter:\n"
            f"    geoip: true\n"
            f"    geoip-code: RU\n"
            f"\n"
            f"rules:\n"
            f"  - DOMAIN-SUFFIX,local,DIRECT\n"
            f"  - IP-CIDR,127.0.0.0/8,DIRECT\n"
            f"  - IP-CIDR,192.168.0.0/16,DIRECT\n"
            f"  - IP-CIDR,10.0.0.0/8,DIRECT\n"
            f"  - IP-CIDR,172.16.0.0/12,DIRECT\n"
            f"  - GEOSITE,category-ru,DIRECT\n"
            f"  - GEOIP,RU,DIRECT\n"
            f"  - MATCH,Proxy\n"
        )
